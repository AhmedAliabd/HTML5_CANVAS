...
function handlePracticals() {
  for (let i = 0; i < listOfPracticals.length; i++) {
    ...
    for (let j = i; j < listOfPracticals.length; j++) {
      const dx = listOfPracticals[i].x - listOfPracticals[j].x;
      const dy = listOfPracticals[i].y - listOfPracticals[j].y;
      const distance = Math.sqrt(dx * dx + dy * dy);
      if (distance < 100) {
        ctx.beginPath();
        ctx.strokeStyle = listOfPracticals[i].color;
        ctx.lineWidth = 0.2;
        ctx.moveTo(listOfPracticals[i].x, listOfPracticals[i].y);
        ctx.lineTo(listOfPracticals[j].x, listOfPracticals[j].y);
        ctx.stroke();
        ctx.closePath();
      }
    }
